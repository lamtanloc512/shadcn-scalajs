...
/** Laminar port of basecoat's `.btn` component (basecoat/src/css/components/button.css
  * + basecoat/src/css/styles/vega.css:20-80 for the `data-variant`/`data-size` contract).
  * Pure CSS/no-JS tier: all visual variants come from the vendored basecoat CSS bundle —
  * this file only emits the right element/class/attributes.
  */
object Button:
  ...
  /** Builder-style usage mirroring the `Component.of(_.prop := value, ...)` pattern
    * already proven by laminar-shoelace / web-components-ui5 in this workspace:
