...
object AlertDialog:
  ...
  private val contentBase: String =
    s"cn-alert-dialog-content group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-6 rounded-xl bg-popover p-6 text-popover-foreground outline-none ring-1 ring-foreground/10 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-md data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 ${Dialog.exitFillClass}"

  def apply(isOpenVar: Var[Boolean])(mods: Modifier[HtmlElement]*): HtmlElement =
    // Upstream's AlertDialog does not dismiss on overlay click; Escape still closes (bits-ui default).
    Dialog.element(
      isOpenVar,
      "alert-dialog",
      "alert-dialog-content",
      contentBase,
      Dialog.Options(dismissOnOutsideClick = false)
    )(
  ...
  def overlay(mods: Modifier[HtmlElement]*): HtmlElement =
    div(
      ...
      cls := s"cn-alert-dialog-overlay fixed inset-0 z-50 bg-black/10 duration-100 supports-backdrop-filter:backdrop-blur-xs data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0 ${Dialog.exitFillClass}",
      mods
    )
