...
/** Hand-rolled reactive port of basecoat's dropdown-menu.js state machine
  * (basecoat/src/js/dropdown-menu.js: roving `aria-activedescendant`,
  * `aria-expanded` open/close, click-outside-closes) onto Airstream
  * Var/EventBus. There is no Radix-equivalent primitives library for
  * Laminar, so this tier of component reimplements the behavior directly,
  * following the Var idioms used in laminar-full-stack-demo's CounterView
  * / FormStateView instead of imperative DOM manipulation.
  */
object DropdownMenu:
    ...
    div(
      ...
      onMountBind { ctx =>
        ...
        documentEvents(_.onMouseDown) --> { (ev: dom.MouseEvent) =>
          if isOpenVar.now() && DropdownMenu.composedPath(ev).indexOf(ctx.thisNode.ref) == -1 then
            isOpenVar.set(false)
        }
      },
      button(
  ...
  private def renderItem(item: Item, idx: Int, activeIdx: Int, itemDomId: Int => String, select: Int => Unit): HtmlElement =
    div(
